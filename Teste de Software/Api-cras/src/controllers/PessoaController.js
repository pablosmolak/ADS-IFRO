  ...
  static listarPessoas = async (req, res) => {
    try {
      return await PermissaoMidleware.verificarPermissao('pessoas', 'get', req, res, async () => {
        ...
        // Buscar tudo por falta de paramentro
        if (!cpf && !nome) {
          const pessoa = await pessoas.paginate({}, options);
          return res.json(pessoa);
        } 
        
        // Buscar por  CPF e NOME ao mesmo tempo
        if (cpf && nome) {
          const pessoa = await pessoas.paginate({$and: [{ cpf: new RegExp(cpf, 'i') }, { nome: new RegExp(nome, 'i')}]}, options);
          return res.json(pessoa);
        } 
        
        // busca por cpf por falta do nome
        if (!nome) {
          const pessoa = await pessoas.paginate({ cpf: new RegExp(cpf, 'i') }, options);
          return res.json(pessoa);
        } 
        // busca por nome por falta de cpf
        if (!cpf) {
