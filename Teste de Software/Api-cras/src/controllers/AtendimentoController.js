  ...
  static listarAtendimentos = async (req, res) => {
    try {
      return await PermissaoMidleware.verificarPermissao('atendimentos', 'get', req, res, async () => {
        ...
        // buscar por tipo e intervalo de datas de atendimento
        if (!oid_pessoa) {
          const atendimento = await atendimentos.paginate({$and: [{ tipo: new RegExp(tipo, 'i') }, { dataAtendimento: { $gte: dataAtendimento, $lte: dataAtendimentoFinal } }]}, options);
          return res.json(atendimento);
        }
        
        // buscar por tipo de atendimento e ID da pessoa atendida
        if (!dataAtendimento) {
          const atendimento = await atendimentos.paginate({$and: [{ tipo: new RegExp(tipo, 'i') }, { oid_pessoa: oid_pessoa}]}, options);
          return res.json(atendimento);
        }
        ...
        if (!dataAtendimento && !dataAtendimentoFinal) {
